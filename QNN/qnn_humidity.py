...
def qnn_humidity():
    ...
    def forward(x, w): # first 3 inputs and weight values
        ...
        bound_qc = qc.assign_parameters(bind_dict) # Parameters with real numbers
        job = estimator.run(
            [(bound_qc, observable)]
        )
        result = job.result() # Runs estimator on circuit and observable
    ...
    X_train, Y_train = X[:split_index], Yhum[:split_index] # training inputs and targets
    X_val, Y_val = X[split_index:], Yhum[split_index:] # validation inputs and targets
    ...
    for epoch in range(epochs):
        ...
        print(f"Humidity, Epoch {epoch}, training loss = {loss_train(w):.6f}, validation loss = {loss_val(w):.6f}") # finds training and validation loss and prints both to 6 decimals
